...
def get_query_params() -> dict:
    ...
    """
    if (
        "raw_query_params" not in st.session_state
        or not st.session_state["raw_query_params"]
    ):
        ...
        st.session_state["raw_query_params"] = params
    return st.session_state["raw_query_params"]
...
def set_query_params(**kwargs):
    ...
    """
    st.session_state["raw_query_params"].update(
        {
    ...
    def wrapper(label: str, *args, **kwargs) -> Any:
        ...
        if url_param is not None:
            if value_type is bool:
                url_value: Any = _string_to_bool(url_param)
        ...
        if sync_key not in st.session_state:
            if url_value is not None:
                st.session_state[sync_key] = url_value
            elif "value" in kwargs:
                st.session_state[sync_key] = kwargs.pop("value")
            elif "index" in kwargs:
                st.session_state[sync_key] = kwargs.pop("index")
            elif "default" in kwargs:
                st.session_state[sync_key] = kwargs.pop("default")

        original_on_change = kwargs.pop("on_change", None)

        def on_change():
            new_value = st.session_state[sync_key]
...
checkbox = url_sync(st.checkbox, bool, False)
radio = url_sync(st.radio, int, 0)
text_input = url_sync(st.text_input, str, "")
text_area = url_sync(st.text_area, str, "")
number_input = url_sync(st.number_input, float)
slider = url_sync(st.slider)
date_input = url_sync(st.date_input, date)
selectbox = url_sync(st.selectbox, int, 0)
multiselect = url_sync(st.multiselect, list)
